...
class ApprovePaymentRequestTest extends TestCase
{
    ...
    public function test_finance_cannot_approve_non_pending_payment_request(PaymentRequestStatus $status): void
    {
        $finance = User::factory()->finance()->create();
        $token = $finance->createToken('auth_token')->accessToken;
        $paymentRequest = PaymentRequest::factory()->create([
        ...
        $response->assertStatus(409)
            ->assertJsonPath('message', 'Only pending payment requests can be approve');
    }
    ...
    public function test_approve_requires_authentication(): void
    {
        ...
        $response = $this->postJson("/api/payment-requests/{$paymentRequest->id}/approve");

        $response->assertStatus(401);
    }

    public function test_approve_returns_not_found_for_unknown_id(): void
    {
        $finance = User::factory()->finance()->create();
        $token = $finance->createToken('auth_token')->accessToken;

        $response = $this->withHeader('Authorization', 'Bearer '.$token)
            ->postJson('/api/payment-requests/999999/approve');
